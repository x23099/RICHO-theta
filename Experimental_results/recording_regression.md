...
| `dynamic-ttc-20260901-1435` | `202609011435.tar.xz` | 30 fpsで0.10 m/s接近・後退、0.20 m/s接近 | 録画完全性PASS、固定TTC条件0/10 PASS、アーカイブ総合FAIL |
...
9月1日の30 fps holdoutは10セッションを保持する。正式9試行は0.10 m/s接近・後退の
`r01`～`r03`と、0.20 m/s接近の`r01`、`r02`、`r04`である。0.20 m/sの`r03`は収録時に
操作内容への不安があり、結果を見る前に追加した`r04`で置換した。アーカイブからは削除せず、
診断試行として回帰対象に残す。正式9試行は固定profileで0/9 PASS、診断`r03`もFAILだった。
このFAILを基準に、今後の評価区間または警告設計変更が実際に差を生むか検出する。
