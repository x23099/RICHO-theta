...
現在の実行環境では`/dev/video*`が見えないため、実カメラが露出固定を受け付けるかは未確認である。

## 5. 次の作業

1. カメラが見える学校PC上でpreflightを実行し、`auto_exposure`、`exposure`、`gain`、
   `auto_white_balance`の値を保存する。
2. `v4l2-ctl --list-ctrls`で露出・ホワイトバランス制御の対応範囲を読み取り確認する。
3. 対応している場合だけ、変更前値を保存してから自動露出と固定露出の短いA/B録画を行う。
4. 対応していない場合は、HSV輪郭方式を延命せず、学習済み物体検出または形状を含む別検出器を
   開発データで比較する。
